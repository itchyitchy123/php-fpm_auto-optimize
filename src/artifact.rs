...
pub fn validate_plan(plan: &Plan) -> Result<()> {
    ...
    for pool in &plan.pools {
        ...
        if !matches!(
            pool.evidence.memory_measurement.as_deref(),
            None | Some("pss" | "rss" | "mixed")
        ) {
            bail!(
                "pool {} has an unknown memory measurement method",
                pool.id.name
            );
        }
        if pool.evidence.status_samples > pool.evidence.status_attempts {
            bail!(
                "pool {} has more successful than attempted status samples",
                pool.id.name
            );
        }
        if pool
            .reasons
            .iter()
            .chain(&pool.evidence.warnings)
            .any(|value| value.chars().any(char::is_control))
        {
            bail!("pool {} contains unsafe diagnostic text", pool.id.name);
        }
        let percentiles = [
            pool.evidence.memory_p50_mb,
            pool.evidence.worker_memory_mb,
            pool.evidence.memory_p95_mb,
            pool.evidence.memory_max_mb,
        ];
        if percentiles
            .into_iter()
            .flatten()
            ...
            .is_none()
        {
            bail!("pool {} memory percentiles are inconsistent", pool.id.name);
        }
    }
